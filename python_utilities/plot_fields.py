...
for n_step in ts_vec:
    ...
    # file_names = ['u_{:08d}.dat', 'v_{:08d}.dat', 'w_{:08d}.dat', 'phi_{:08d}.dat', 'T_{:08d}.dat']
    # file_names = ['u_{:08d}.dat']

    coord_files = ['x.dat', 'y.dat', 'z.dat']

    # Add the path string to file names
    file_names = [file_name.format(n_step) for file_name in file_names]
    coord_files = [foldername + coord_file for coord_file in coord_files]

    # Load the coordinate arrays
    # x = np.fromfile(coord_files[0], dtype=np.float64)
    # y = np.fromfile(coord_files[1], dtype=np.float64)
    # z = np.fromfile(coord_files[2], dtype=np.float64)
    # z = np.flip(z)
    ...
    if timeaverage == 0 or n_step == ts_vec[-1]:
        if showmaps_flag == 1:
            for i, array in enumerate(data_arrays):
                ...
                resampled = griddata( aaa, array.T.flatten(), (hors, vers), method='linear')
                ...
                # plt.title(file_names[i],fontsize=fontsize_val)
    ...
    #     plt.plot(vertical_profile, ver, label=file_names[i], alpha=0.7)
    ...
    for i, array in enumerate(data_arrays):
        ...
        # # Plot the vertical profile with a different color for each array
        # plt.plot(vertical_profile[i], ver, label=file_names[i], alpha=0.7)
...
if fluct == 1:
    for i, array in enumerate(data_arrays):
        ...
        cax.xaxis.set_ticks_position('top')
        plt.xticks(fontsize=fontsize_val, rotation=0)
        plt.title(file_names[i]+dersuff+'_RMS',fontsize=fontsize_val)
